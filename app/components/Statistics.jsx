  ...
  tracks.forEach((value, key) => {
    value.artist_names.forEach((artist) => {
      if (!artistPlayedMetrics.has(artist)) {
        ...
        artistPlayedMetrics.set(artist, (artistPlayedMetrics.get(artist) + value.played_at.length));
      }
    });
  });
  ...
  tracks.forEach((value, key) => {
    value.artist_names.forEach((artist) => {
      if (!artistOverallMetrics.has(artist)) {
        artistOverallMetrics.set(artist, (value.played_at.length + value.playlists_added_to.length));
      } else {
        artistOverallMetrics.set(artist, (artistOverallMetrics.get(artist) + (value.played_at.length) + (value.playlists_added_to.length)));
      }
    });
  });
  ...
  tracks.forEach((value, key) => {
    value.genres.forEach((genre) => {
      if (!genresAddedMetrics.has(genre)) {
        ...
        genresAddedMetrics.set(genre, (genresAddedMetrics.get(genre) + value.playlists_added_to.length));
      }
    });
  });
  ...
  tracks.forEach((value, key) => {
    value.genres.forEach((genre) => {
      if (!genresPlayedMetrics.has(genre)) {
        ...
        genresPlayedMetrics.set(genre, (genresPlayedMetrics.get(genre) + value.played_at.length));
      }
    });
  });
  ...
  tracks.forEach((value, key) => {
    value.genres.forEach((genre) => {
      if (!genresOverallMetrics.has(genre)) {
        genresOverallMetrics.set(genre, (value.played_at.length + value.playlists_added_to.length));
      } else {
        genresOverallMetrics.set(genre, (genresOverallMetrics.get(genre) + value.played_at.length + value.playlists_added_to.length));
      }
    });
  });
...
export default function Statistics({ user }) {
  ...
  const artistOverallMetrics = getArtistOverallMetrics(tracks);
